...
import PortfolioPosts from './PortfolioPosts';
import { Link } from 'react-router-dom';

class PortfolioPage extends TabPage {
  ...
  componentDidMount() {
    ...
    socket.on(`server > client portfolio page update followers`, (followers) => {
        let {page} = this.state;
  ...
  componentWillMount(){
      ...
      socket.emit('client > server get tabPage', 'portfolio', this.updatePage);
  }
  ...
  render() {
            ...
            <div className="tpSettingsCont">
                <div className="mainWrap">
                    <div className="tpSettingsBtns">
                        <Link to="/portfolio/new" className="btn primary">new portfolio post</Link>
                    </div>
                </div>
            </div>
        );
    ...
    return (
      <>
        ...
        {newPostComp}
        {
            (currentPage == 1 && user.preferences.showFeaturedPortfolioPosts)?(
